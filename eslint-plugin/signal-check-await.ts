...
function isComputedType(type: Type): boolean {
  ...
  return sourceFile.fileName.includes('ccstate') && symbol.getName() === 'computed'
}
...
export const signalCheckAwait = createRule({
  ...
  create(context) {
    ...
    const awaitExpressionsMap = new Map<TSESTree.Node, TSESTree.AwaitExpression | null>()
      ...
      'ExpressionStatement, VariableDeclaration'(
        ...
        if (node.type === AST_NODE_TYPES.VariableDeclaration) {
          for (const declarator of node.declarations) {
            ...
            const safePromises = safePromisesMap.get(parent) ?? new Set<string>()
            ...
            if (declarator.init.type === AST_NODE_TYPES.CallExpression) {
              ...
              isSafe = isSafePromise(sourceCode, safePromisesMap, declarator.init)
            }
