...
const deferredCallbackFunctions = new Set(['setInterval', 'setTimeout'])
...
function isFunctionNode(node: TSESTree.Node): node is FunctionNode {
  return isCallbackNode(node) || node.type === AST_NODE_TYPES.FunctionDeclaration
}
...
function isDeferredCallbackCall(call: TSESTree.CallExpression, callback: CallbackNode): boolean {
  if (!call.arguments.includes(callback)) {
    return false
  }
  return (
    call.callee.type === AST_NODE_TYPES.Identifier &&
    deferredCallbackFunctions.has(call.callee.name)
  )
}
...
function callbackEscapes(primitive: 'command' | 'computed', node: FunctionNode): boolean {
  if (primitive !== 'command' || !isCallbackNode(node)) {
    return false
  }
...
function bindingFromPropertyValue(value: TSESTree.Property['value']): TSESTree.Identifier | null {
...
function commandAccessorBindings(callback: CallbackNode): TSESTree.Identifier[] {
...
function computedAccessorBindings(callback: CallbackNode): TSESTree.Identifier[] {
  const firstParam = callback.params[0]
  return firstParam?.type === AST_NODE_TYPES.Identifier ? [firstParam] : []
}

export const noAccessorEscape = createRule({
  name: 'no-accessor-escape',
  defaultOptions: [],
  meta: {
    type: 'problem',
    docs: {
      description: 'prevent ccstate get/set accessors from escaping command()/computed() callbacks',
    },
    schema: [],
    messages: {
      accessorEscape:
  ...
  create(context) {
    function checkCallback(primitive: 'command' | 'computed', callback: CallbackNode) {
      const bindings =
        primitive === 'command'
      ...
      for (const variable of variables) {
        ...
        for (const reference of variable.references) {
          ...
          if (
            ...
            !isAllowedAccessorUse(primitive, identifier, callback)
          ) {
            context.report({
              node: identifier,
