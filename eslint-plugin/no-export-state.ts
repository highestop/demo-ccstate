...
export const noExportState = createRule({
  ...
  create(context) {
    ...
    function isStateType(type: Type): boolean {
      ...
      return sourceFile.fileName.includes('ccstate') && symbol.getName() === 'State'
    }

    function checkExportedNode(node: TSESTree.Node | null | undefined, name: string) {
      if (!node) {
        return
      }
    ...
    return {
      ...
      'ExportNamedDeclaration > ExportSpecifier'(node: TSESTree.ExportSpecifier) {
